...
- UI 라이브러리: Tailwind
    - 상태 관리: React Context + usereducer, Zustand
    (로컬 스토리지 연동)
    - 테스트: Jest, React Testing Library
- 백엔드: Node.js(TypeScript)
- 인프라: AWS 서버리스 (API Gateway, Lambda, DynamoDB) 아키텍처 사용
- 빌드 도구: Vite 
...
1. 백엔드 서버가 필요한가요, 아니면 클라이언트 측 저장만으로 충분한가요?
    - 처음에는 클라이언트 저장을 쓰고 백엔드 구현이 되면 백엔드에 연결
2. 오프라인 기능이 필요한가요?
    - 필요 없음
...
1. 배포 대상 플랫폼은 무엇인가요? (웹 서버, GitHub Pages, Netlify 등)
    - AWS에 배포, 프론트엔드와 CI/CD는 Github Pages와 Github Actions 사용
...
1. 기본 TODO CRUD 기능 구현
    - 처음에는 프론트만으로 동작하도록 구현하고 백엔드 구현이되면 백엔드에 연결
2. UI/UX 개선
3. 필터링 및 정렬 기능 구현
4. 데이터 관리 기능 구현
